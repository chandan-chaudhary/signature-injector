...
import { Type, PenTool, Image, Calendar, CircleDot, X } from "lucide-react";
...
export function DraggableField({
  ...
  const renderFieldContent = () => {
    switch (field.type) {
      case "text":
        return field.value ? (
          <span className="text-sm text-foreground truncate px-1">
            {field.value}
          </span>
        ) : (
  ...
  return (
    <Rnd
      ...
      // Ensure the draggable element is absolutely positioned inside the overlay
      ...
      className={cn("group", isSelected && "z-50")}
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
