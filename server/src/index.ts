...
import { calculateSHA256, verifySHA256 } from "./utils/crypto.js";
...
const MONGO_URI =
  process.env.MONGO_URI || "mongodb://localhost:27017/signatures";
...
app.post("/sign-pdf", async (req: Request, res: Response) => {
  try {
    const { pdfBase64, signatureBase64, coordinates } = req.body as {
      pdfBase64?: string;
    ...
    if (
      [x, y, width, height, pageRenderWidth, pageRenderHeight].some(
        (v) => v === undefined
      )
    ) {
      return res.status(400).json({
        error:
          "coordinates must include x,y,width,height,pageRenderWidth,pageRenderHeight",
      });
    }

    // Parse PDF base64 (handle data URI or raw base64)
    let rawPdfBase64 = pdfBase64;
    const pdfDataUriMatch = pdfBase64.match(
    ...
    let imageType = "png";
    const dataUriMatch = signatureBase64.match(
    ...
    const targetPageIndex = Math.max(
      0,
      Math.min(pdfDoc.getPageCount() - 1, page - 1)
    );
    ...
    const scale = Math.min(
      boxWidthPdf / imgWidthPx,
      boxHeightPdf / imgHeightPx,
      1
    );
...
app.post("/verify-document", async (req: Request, res: Response) => {
  try {
    ...
    if (isOriginal) {
      ...
      return res.json({
        verified: false,
        message:
          "Document hash does not match any record - document may have been tampered with",
        hash: calculatedHash,
 ...
 */
app.get("/audit-trail/:recordId", async (req: Request, res: Response) => {
  try {
